...
best20Model = glm(paste0(best20[,5]),family=binomial(link='logit'),
                           data=meanStainWide_Tranformed)
model_20VIF = best20Model # Save for later
...
While working on the cell segmentation we discovered that for 10 patients no Ki67 measurements were taken and instead that column contained the B7H4 stain. Let's check if this influences the model conclusions.

Load the data
<<results='markup'>>=
# I changed the stain statistics collection method so that the data is already in the right format.
meanStain_Corrected = read.csv("rawStainSummaries_corrected.csv")
# Standardise the data, using the caret preproces function. Note: preProcess can also do a Box-Cox transform
require(caret)
meanStainCorrected_Tranformed = meanStain_Corrected
preprocessParams = preProcess(meanStain_Corrected[,3:dim(meanStain_Corrected)[2]],
                              method=c("center", "scale"),verbose=T)
meanStainCorrected_Tranformed[,3:dim(meanStain_Corrected)[2]] =
  predict(preprocessParams, meanStain_Corrected[,3:dim(meanStain_Corrected)[2]])
@

% -------------------------------------------------------------
\subsection{Model Selection with Ki67}
We can't just use \texttt{step()} now because if Ki67 is dropped as a variable the amount of data used changes. If Ki67 is included, the cores without Ki67 have to be ignored. If it's dropped they can be included. In the earlier analyses Ki67 never showed up as particularly important, so it might just be fine to drop it in general. To gather some evidence for this let's do an analysis where we drop all the patients which don't have Ki67, so that we have a full data set.

<<results='markup'>>=
omit = c(1)
initModel = glm(PtSnty ~.,family=binomial(link='logit'),
                           data=meanStainCorrected_Tranformed[!is.na(meanStainCorrected_Tranformed$Ki67),-omit])
reducedCoLinModelArr100 = AICVIFCoElimination(DecorrelateVariables(initModel,100,verbose=F)
                                              ,verbose=F)
reducedCoLinModelArr20 = AICVIFCoElimination(DecorrelateVariables(initModel,20,verbose=F)
                                             ,verbose=F)
...
Say we tolerate a maximum VIF of 10. What are the best AICs we get?
...
Does any of them contain Ki67?

<<results='markup'>>=
print(paste(best100$formulaString))
print(best20$formulaString)
print(best10$formulaString)
@

Neither of them has kept the Ki67 column. Let's drop it.

% -------------------------------------------------------------
\subsection{Model Selection without Ki67 on 121 Cores}
Now that we dropped Ki67 let's see if we still get the same results as before I detected the mix up.
<<results='markup'>>=
omit = c(1,which(names(meanStainCorrected_Tranformed)=="Ki67"))
initModel = glm(PtSnty ~.,family=binomial(link='logit'),
                           data=meanStainCorrected_Tranformed[,-omit])
reducedCoLinModelArr100 = AICVIFCoElimination(DecorrelateVariables(initModel,100,verbose=F)
                                              ,verbose=F)
reducedCoLinModelArr20 = AICVIFCoElimination(DecorrelateVariables(initModel,20,verbose=F)
                                             ,verbose=F)
...
Say we tolerate a maximum VIF of 25 as before. Do we get the same model?
...
No, the previous model had an AIC of 135 or 145 and between 12 or 13 variables...

Let's look at the models.

<<Fig_Model100_VF25,tidy=F,results='markup',fig.pos="h", fig.cap="Importance of the different stains according to the logistic model with maxVIF 10. Asterisk indicates level of statistical support for non-zero contribution from this stain (T-test: *p$<$0.05,**p$<$0.01).">>=
best100Model = glm(paste0(best100[,5]),family=binomial(link='logit'),
                           data=meanStainCorrected_Tranformed[,-omit])
PlotCoefficients(best100Model,yLim=c(-30,30),yPos=22,errBarWidth=.4)
@


<<Fig_Model20_VF25,tidy=F,results='markup',fig.pos="h", fig.cap="Importance of the different stains according to the logistic model with maxVIF 20. Asterisk indicates level of statistical support for non-zero contribution from this stain (T-test: *p$<$0.05,**p$<$0.01).">>=
best20Model = glm(paste0(best20[,5]),family=binomial(link='logit'),
                           data=meanStainCorrected_Tranformed[,-omit])
PlotCoefficients(best20Model,yLim=c(-30,30),yPos=22,errBarWidth=.4)
@


<<Fig_Model10_VF25,tidy=F,results='markup',fig.pos="h", fig.cap="Importance of the different stains according to the logistic model with maxVIF 9.2, aic 141 and accuracy 0.8. Asterisk indicates level of statistical support for non-zero contribution from this stain (T-test: *p$<$0.05,**p$<$0.01).">>=
best10Model = glm(paste0(best10[,5]),family=binomial(link='logit'),
                           data=meanStainCorrected_Tranformed[,-omit])
p = PlotCoefficients(best10Model,yLim=c(-50,50),yPos=22,errBarWidth=.4)
# Annotate the markers
yPos = 37
tSize = 2.5
# Positive
...
p = p + annotate("text", x = "HistoneH3", y = yPos,
                       label = "Generic Cell Marker", size = tSize)
p = p + annotate("text", x = "CollagenI", y = yPos,
                       label = "ECM", size = tSize)
p = p + annotate("text", x = "CD44s", y = yPos,
                       label = "Cell-Cell Interaction", size = tSize)
p = p + annotate("text", x = "B7.H4", y = yPos,
                       label = "Immune Checkpoint \n on Antigen \n Presenting Cells", size = tSize)
p = p + annotate("text", x = "p21", y = yPos,
                       label = "Cell Cycle Inhibitor", size = tSize)
p = p + annotate("text", x = "DNA191", y = yPos,
                       label = "DNA", size = tSize)
p = p + geom_rect(aes(xmin = "pS6", xmax = 14.5, ymin = 25, ymax = 50),
               fill = "transparent", color = "green4", size = 1.5)
...
p = p + annotate("text", x = "CD152", y = yPos,
                       label = "Checkpoint Inhibitor", size = tSize)
p = p + geom_rect(aes(xmin = 0, xmax = "DNA191", ymin = -25, ymax = -50),
               fill = "transparent", color = "red", size = 1.5)

p
@

They're slightly different but all teling more or less a similar story. I think to really make this more insightful we should either i) choose stains by hand, or/and, ii) decide what we want the model to do. Do we want the stains that have most discriminative power, or do we want a model that is optimised for prediction.

Something else I've been thinking is how suitable VIFs are when the data is highly correlated. Since we're fitting linear models wouldn't we be suffering the same problem of it being difficult to assign the variance to a particular variable? Perhaps in this case the covariance matrix approach is more suitable?

% =======================================================
\section{A more Systematic Approach}
While working on the cell segmentation we discovered that for 10 patients no Ki67 measurements were taken and instead that column contained the B7H4 stain. Let's check if this influences the model conclusions.
...
Let's plot the correlation between variables.

<<Fig_CovPlot,tidy=F,results='markup',fig.pos="h", fig.cap="Plot of the covariates against each other.">>=
plot(meanStainCorrected_Tranformed[,3:8])
@

This makes it pretty obvious that there is a lot of correlation between the different covariates.

Let's look at the covariance matrix.

<<Fig_CovMatPlot,tidy=F,results='markup',fig.pos="h", fig.cap="Heatmap of the covariance matrix.">>=
require(graphics); require(grDevices)
colScheme = rainbow(36, start = 0, end = .3)
omit = c(1,2,which(names(meanStainCorrected_Tranformed)=="Ki67"))
coMat = cor(meanStainCorrected_Tranformed[,-omit])
heatmap(coMat,ColSideColors=colScheme,symm=TRUE)
@

It looks like beta-catenin and B7H3 are very highly correlated with a lot of other stuff. Let's look at if that matches the VIFs:

<<results='markup'>>=
omit = c(1,which(names(meanStainCorrected_Tranformed)=="Ki67"))
fullModel = glm(PtSnty ~.,family=binomial(link='logit'),
                           data=meanStainCorrected_Tranformed[,-omit])
vifVec = vif(fullModel)
vifVec[order(vifVec,decreasing=T)]
@

Ah, so that flags up the DNA and RR stains. Let's remove them and let's also remove the background control.

<<results='markup'>>=
omit = c(1,which(names(meanStainCorrected_Tranformed)=="Ki67"),
         which(names(meanStainCorrected_Tranformed)=="DNA193"),
         which(names(meanStainCorrected_Tranformed)=="RR101"),
         which(names(meanStainCorrected_Tranformed)=="SrBCK"),
         which(names(meanStainCorrected_Tranformed)=="XeBCK"))
fullModel = glm(PtSnty ~.,family=binomial(link='logit'),
                           data=meanStainCorrected_Tranformed[,-omit])
vifVec = vif(fullModel)
vifVec[order(vifVec,decreasing=T)]
...
That massively reduces the VIFs. What does it do to the covariance matrix?

<<Fig_CovMatPlot_Rem1,tidy=F,results='markup',fig.pos="h", fig.cap="Heatmap of the updated covariance matrix.">>=
require(graphics); require(grDevices)
coMat = cor(meanStainCorrected_Tranformed[,-omit])
colScheme = rainbow(ncol(meanStainCorrected_Tranformed[,-omit]), start = 0, end = .3)
heatmap(coMat,ColSideColors=colScheme,symm=TRUE)
@

Slightly redder. Let's reduce the correlation using the correlation matrix instead of VIFs.

<<Fig_RedCovPlot,tidy=F,results='markup',fig.pos="h", fig.cap="Heatmap of the covariance matrix after co-linearity reduction.">>=
library(caret)
covariatesArr = meanStainCorrected_Tranformed[,-c(omit,2)]
coMat = cor(covariatesArr)
hc = findCorrelation(coMat,cutoff=0.7,exact=TRUE) # put any value as a "cutoff" 
hc = sort(hc)
meanStain_Reduced = data.frame(meanStainCorrected_Tranformed[,c(1,2)],covariatesArr[,-c(hc)])
dim(meanStain_Reduced)
coMat = cor(meanStain_Reduced[,-c(1,2)])
heatmap(coMat,symm=TRUE)
@
...
<<results='markup'>>=
fullModel = glm(PtSnty ~.,family=binomial(link='logit'),
                           data=meanStain_Reduced[,-c(1)])
vifVec = vif(fullModel)
vifVec[order(vifVec,decreasing=T)]
@

They're now down quite significantly. Mostly CD16 is problematic. What does it depend on?

<<results='markup'>>=
cd16Model = lm(CD16 ~.,data=meanStain_Reduced[,-c(1,2)])
summary(cd16Model)
@

CD163 comes out, which is also a macrophage marker. So this makes sense. Let's take CD16 instead of CD163.

<<results='markup'>>=
omit = c(omit,which(names(meanStain_Reduced)=="CD163"))
fullModel = glm(PtSnty ~.,family=binomial(link='logit'),
                           data=meanStain_Reduced[,-omit])
vifVec = vif(fullModel)
vifVec[order(vifVec,decreasing=T)]
@

We're now at acceptable VIFs. Let's do step
<<results='markup'>>=
initModel = glm(PtSnty ~.,family=binomial(link='logit'),
                           data=meanStain_Reduced[,-omit])
finalModel = step(initModel,trace=0)
summary(finalModel)
vifVec = vif(finalModel)
vifVec[order(vifVec,decreasing=T)]

@

<<Fig_AltColRed,tidy=F,results='markup',fig.pos="h", fig.cap="Model from alternative variance reduction. Asterisk indicates level of statistical support for non-zero contribution from this stain (T-test: *p$<$0.05,**p$<$0.01).">>=
p = PlotCoefficients(finalModel,yLim=c(-50,50),yPos=22,errBarWidth=.4)
# Annotate the markers
yPos = 37
tSize = 2.5
# Positive
...
p = p + annotate("text", x = "CD44s", y = yPos,
                       label = "Cell-Cell Interaction", size = tSize)
p = p + annotate("text", x = "CD14", y = yPos,
                       label = "Macrophages /\n Dendritic Cells", size = tSize)
p = p + geom_rect(aes(xmin = "pS6", xmax = 14.5, ymin = 25, ymax = 50),
               fill = "transparent", color = "green4", size = 1.5)
...
p = p + annotate("text", x = "CD134", y = yPos,
                       label = "T-Cell Activation\n Survival", size = tSize)
p = p + geom_rect(aes(xmin = 0, xmax = "CD14", ymin = -25, ymax = -50),
               fill = "transparent", color = "red", size = 1.5)
...
% =======================================================
\section{A predictive vs explanatory approach}
From the analysis so far it seems like it will be difficult to find a model that is both very predictive but also not too highly co-linear. I believe this might be because some stains are useful for some cores but not for others. For example, one Macrophage stain might have worked on 100 cores but not on the remaining 21 and similarly for a different macrophage stain. Then for most of the patients the two contain the same information, except for the few where one is more helpful than the other. The predictive model would then keep both, whereas an explanatory model would drop one of them. Let's try this approach here.

We will proceed as follows: We de-correlate the data up to a certain VIF (100,20,10) and then do the AIC, VIF co-optimisation. Subsequently we pick the model with the best AIC below a certain VIF. For the predictive model we set this threshold at 20. For the explanatory model we set it at 5.

% -----------------------------------------------------
\subsection{De-correlation and Step Search}
Let's begin with de-correlation and step search. Since people seem to be using VIFs quite heavily I will assume that they work. The correlation matrix based approach seemed not to change very much. Maybe can do that more systematically again and put it in the appendix, but VIF based reduction seems the way to go for now.
...
Let's find the predictive model, where we allow a max VIF of 20.
<<results='markup'>>=
targetVIF = 20
best100 = reducedCoLinModelArr100[unlist(reducedCoLinModelArr100$maxVIF)<targetVIF,]
best100 = best100[which.min(unlist(best100$V1)),]
...
How do they do in cross-validation?

<<Fig_PredModelComp,tidy=F,results='markup',fig.pos="h", fig.cap="Cross-validation results for the three different predictive models. The two models with 14 variables do best.">>=
# Cross-validation
source("../Utils_Maxi.R")
modelVec = c(paste0(best100[,5]),paste0(best20[,5]),paste0(best10[,5]))
labelVec = c("7 Variables, VIF: 10.1, AIC: 146.7",
             "14 Variables, VIF: 20, AIC: 141.7",
             "14 Variables, VIF: 9.2, AIC: 141.7")
PlotCrossValidation(modelVec,meanStainCorrected_Tranformed[,-omit],nIter=100,nFolds=5,labelVec=labelVec)
@

The two with 14 variables do better than the one with 7 variables, but both do roughly equally well (around 71\%). Interestingly the overall performance is much lower than what was achieved with the ``messed-up'' data. Maybe this is because of the B7H4 signal that both are picking up now.

Given that both Model20 and Model10 seem equally predictive I will choose Model10 as it has the lower VIF.

<<Fig_PredModCoef,tidy=F,results='markup',fig.pos="h", fig.cap="Coefficients of the model with maximised predictive power. Asterisk indicates level of statistical support for non-zero contribution from this stain (T-test: *p$<$0.05,**p$<$0.01).">>=
predictiveModel = glm(paste0(best10[,5]),family=binomial(link='logit'),
                           data=meanStainCorrected_Tranformed[,-omit])

p = PlotCoefficients(predictiveModel,yLim=c(-50,50),yPos=22,errBarWidth=.4)
# Annotate the markers
yPos = 37
tSize = 2.5
# Positive
...
% -----------------------------------------------------
\subsection{The Explanatory Model}
Let's find the explanatory model, where we allow a max VIF of 5.
<<results='markup'>>=
targetVIF = 5
best100 = reducedCoLinModelArr100[unlist(reducedCoLinModelArr100$maxVIF)<targetVIF,]
best100 = best100[which.min(unlist(best100$V1)),]
...
<<Fig_ExpModelComp,tidy=F,results='markup',fig.pos="h", fig.cap="Cross-validation results for the three different explanatory models. The two models with 14 variables do best.">>=
# Cross-validation
modelVec = c(paste0(best100[,5]),paste0(best20[,5]),paste0(best10[,5]))
labelVec = c("6 Variables, VIF: 3.8, AIC: 149.8",
             "10 Variables, VIF: 4.7, AIC: 147.47",
             "10 Variables, VIF: 4.7, AIC: 147.47")
PlotCrossValidation(modelVec,meanStainCorrected_Tranformed[,-omit],nIter=100,nFolds=5,labelVec=labelVec)
@

The two with 10 variables do better than the one with 6 variables, but both do roughly equally well (around 69\%). AIC would dictate Model20 or Model10 here

<<Fig_ExplModCoef,tidy=F,results='markup',fig.pos="h", fig.cap="Coefficients of the model with VIF<5 and best predictive power (The ``explanatory model''). Asterisk indicates level of statistical support for non-zero contribution from this stain (T-test: *p$<$0.05,**p$<$0.01).">>=
explanatoryModel = glm(paste0(best10[,5]),family=binomial(link='logit'),
                           data=meanStainCorrected_Tranformed[,-omit])

p = PlotCoefficients(explanatoryModel,yLim=c(-50,50),yPos=22,errBarWidth=.4)
# Annotate the markers
yPos = 37
tSize = 2.5
# Positive
p = p + annotate("text", x = "E.cadherin", y = yPos,
                       label = "Epithelial \n Phenotype", size = tSize)
p = p + annotate("text", x = "HistoneH3", y = yPos,
                       label = "Generic \n Cell Marker", size = tSize)
p = p + annotate("text", x = "CD44s", y = yPos,
                       label = "Cell-Cell \n Interaction", size = tSize)
p = p + annotate("text", x = "CollagenI", y = yPos,
                       label = "ECM", size = tSize)
p = p + annotate("text", x = "B7.H4", y = yPos,
                       label = "Immune Checkpoint \n on Antigen \n Presenting Cells", size = tSize)
p = p + annotate("text", x = "DNA191", y = yPos,
                       label = "DNA", size = tSize)
p = p + geom_rect(aes(xmin = "Vimentin", xmax = 10.5, ymin = 25, ymax = 50),
               fill = "transparent", color = "green4", size = 1.5)
...
% -----------------------------------------------------
\subsection{More Detailed Performance Assessment}
During cross-validation I'm currently only comparing the models against each other. However, it is somewhat difficult from this to tell how well we're really doing. Are we really better than a coin-toss? And if so, by how much? I know that deviance and R2 should tell me information in this direction, but I can't quite interpret them yet and they seem to have their own restrictions. So, I think it will be insightful to actually run such models on our data and directly compare them to the models I've come up with. Let's do this here.

<<Fig_PredvsExpvsCTvsAll1,tidy=F,results='markup',fig.pos="h", fig.cap="Cross-validation performance of the predictive and the explanatory model compared to two benchmarks. The first benchmark is a coin toss (Choosing either outcome at random). The second benchmark is a model which always chooses 1. This shows both our models do better than the benchmarks. Asterisk indicates level of statistical support for non-zero contribution from this stain (T-test: *p$<$0.05,**p$<$0.01).">>=
nIter = 100
nFolds = 5

# Do the cross validation
predModelResultsVec = LogisticCrossVal(nIter,nFolds,predictiveModel$formula,
                                 meanStainCorrected_Tranformed)
explModelResultsVec = LogisticCrossVal(nIter,nFolds,explanatoryModel$formula,
                                 meanStainCorrected_Tranformed)
coinTossModelResultsVec = LogisticCrossVal(nIter,nFolds,predictiveModel$formula,
                                 meanStainCorrected_Tranformed,coinTossModel=T)
always1ModelResultsVec = LogisticCrossVal(nIter,nFolds,predictiveModel$formula,
                                 meanStainCorrected_Tranformed,always1Model=T)


# Plot the results
xValidResult = data.frame(Accuracy=c(predModelResultsVec,explModelResultsVec,
                                     coinTossModelResultsVec,always1ModelResultsVec),
                          Model=as.factor(c(rep(0,length(predModelResultsVec)),
                                            rep(1,length(explModelResultsVec)),
                                            rep(2,length(coinTossModelResultsVec)),
                                            rep(3,length(always1ModelResultsVec)))))
ggplot(xValidResult,aes(x=Model,y=Accuracy,fill=Model)) +
  geom_boxplot() +
  xlab("") +
  scale_fill_discrete(breaks=seq(0,3),labels=c("Predictive Model",
                                               "Explanatory Model",
                                               "Coin-Toss Model",
                                               "Always 1 Model"))
@
